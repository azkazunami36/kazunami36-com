import { Dexie } from "./dexie/dexie-es.js";
/**
...
export class databaseManager {
    ...
    constructor() {
        class easyDB {
            ...
            ;
            async read(path) { return await this.db.main.get({ path: path }); }
            ;
            async write(json) { await this.db.main.put(json); }
            ;
            async delete(path) {
    ...
    async rootCheck() {
        if (!await this.checkIs("/", "Folder"))
            await this.easyDB.write({ path: "/", json: JSON.stringify({ type: "Folder", index: [] }) });
    }
    ;
    /**
     * 検証し一致するかを確認します。
     ...
     * @returns
     */
    async checkIs(path, type) { return (await this.check(path)) === type; }
    ;
    /**
     * 検証します。
     * @param path パスを入力
    ...
    async check(path) {
        ...
        if (infoRaw === undefined)
            return "noexist";
        if (infoRaw.json === undefined)
            return "breaked";
        let errorIs = false;
        const infoJSON = (() => { try {
            return JSON.parse(infoRaw.json);
        }
        catch (e) {
            errorIs = true;
        } ; })();
        if (errorIs || typeof infoJSON !== "object" || infoJSON.type === undefined)
            return "breaked";
    ...
    async mkdir(path) {
        ...
        const infoJSON = (() => { try {
            ...
            throw new Error("パス情報が壊れています。");
        } ; })();
        if (typeof infoJSON !== "object" || infoJSON.type === undefined)
            throw new Error("パス情報が壊れています。");
        if (infoJSON.type !== "Folder")
            throw new Error("フォルダではありません。");
        if (infoJSON.index === undefined || typeof infoJSON.index !== "object" || infoJSON.index.length === undefined)
            throw new Error("パス情報が壊れています。");
        const newFolder = await this.easyDB.read(path);
        if (newFolder !== undefined && newFolder.json !== undefined) {
            const infoJSON = (() => { try {
                return JSON.parse(newFolder.json);
            }
            catch (e) {
                throw new Error("パス情報が壊れています。");
            } ; })();
        ...
        ;
        if (!infoJSON.index.includes(folderName))
            infoJSON.index.push(folderName);
    ...
    async read(path) {
        ...
        if (infoJSON.data === undefined || typeof infoJSON.data !== "string")
            throw new Error("パス情報が壊れています。");
    ...
    async write(path, data) {
        ...
        if (newFile !== undefined && newFile.json !== undefined) {
            const infoJSON = (() => { try {
                return JSON.parse(newFile.json);
            }
            catch (e) {
                throw new Error("パス情報が壊れています。");
            } ; })();
    ...
    async delete(path) {
        ...
        if (infoJSON.type === "Folder") {
            if (infoJSON.index === undefined || typeof infoJSON.index !== "object" || infoJSON.index.length === undefined)
                throw new Error("パス情報が壊れています。");
            for (const fileName of infoJSON.index)
                await this.delete(path + (path[path.length - 1] === "/" ? "" : "/") + fileName);
